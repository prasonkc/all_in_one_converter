...
def yt_download():
    if request.method == "GET":
        ...
        if format == "mp4":
            ...
            converted_dir, filename = download_ytVideo(url, resolution)
            print(filename)
            return jsonify(converted_dir=converted_dir, filename=filename)
        elif format == "mp3":
            bitrate = request.form.get("bitrate")
            converted_dir, filename = download_ytAudio(url, bitrate)
            return jsonify(converted_dir=converted_dir, filename=filename)
